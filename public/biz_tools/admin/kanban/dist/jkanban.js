    ...
    this.addBoards = function (boards, isInit) {
      ...
      for (var boardkey in boards) {
        ...
        headerBoard.innerHTML =
          '<div class="kanban-title-board">' + board.title + '<span style="float:right;color:white" class="deleteBoard" data-id="'+ board.id +'"><i class="fa fa-trash-alt"></i></span>' + ' <span style="float:right;margin-right:5px;color:white;" class="addToBoard" data-id="'+ board.id +'"><i class="fa fa-plus-circle"></i></span> &nbsp;&nbsp;' + '</div>'
        //content board
        ...
        for (var itemkey in board.item) {
          ...
          edit.classList.add("editLeadFromList");
          edit.setAttribute("style", "float:right;display:inline;");
    ...
    function __buildItemTitle (title) {
      ...
      if (self.options.itemHandleOptions.enabled) {
        if ((self.options.itemHandleOptions.customHandler || undefined) === undefined) {
          ...
          result = '<div class=\'item_handle ' + customCssHandler + '\'><i class=\'item_handle ' + customCssIconHandler + '\'></i></div>' + result + '</div>'
        } else {
