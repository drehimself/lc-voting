<form wire:submit.prevent="createChallenge" action="#" method="POST" class="space-y-4 px-4 py-6"
enctype="multipart/form-data">
    <x-alerts></x-alerts>
    <div>
        <input wire:model.defer="title" type="text" class="w-full text-sm bg-gray-100 border-none rounded-xl placeholder-gray-900 px-4 py-2" placeholder="Your Challenge" required>
        @error('title')
    ...
    @enderror
    <div>
        <textarea wire:model.defer="description" name="challenge" id="challenge" cols="30" rows="4" class="w-full bg-gray-100 rounded-xl border-none placeholder-gray-900 text-sm px-4 py-2" placeholder="Describe your challenge" required></textarea>
        @error('description')
            <p class="text-red text-xs mt-1">{{ $message }}</p>
        @enderror
    </div>
