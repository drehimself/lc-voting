    ...
    <div class="idea-container bg-white rounded-xl flex mt-4">
        <div class="flex flex-col md:flex-row flex-1 px-4 py-6">
            <div class="flex-none mx-2 md:mx-4">
                <a href="#">
                    <img src="{{ $challenge->user->getAvatar() }}" alt="avatar" class="w-14 h-14 rounded-xl">
                </a>
            </div>
            <div class="w-full mx-2 md:mx-4">
                <h4 class="text-xl font-semibold">
                    <a href="#" class="hover:underline">{{ $challenge->title }}</a>
                </h4>
                <div class="text-gray-600 mt-3">
                    {{ $challenge->description }}
                </div>
                @if ($challenge->files != '')
                    <img src="{{ asset('storage/'.$challenge->files) }}" alt="" x-data="{}" class="md:object-scale-down h-10 mt-5 inset-0.5"
                    @click="$dispatch('show-image-modal',{image : $event.target.getAttribute('src') })">
                @endif
                <div class="flex flex-col md:flex-row md:items-center justify-between mt-6">
                    <div class="flex items-center text-xs text-gray-400 font-semibold space-x-2">
                        <div class="hidden md:block font-bold text-gray-900">{{ $challenge->user->name }}</div>
                        <div class="hidden md:block">&bull;</div>
                        <div>{{ $challenge->created_at->diffForHumans() }}</div>
                        <div>&bull;</div>
                        <div>{{ $challenge->category->name }}</div>
                        @auth
                            <div>&bull;</div>
                            <div x-data="{isFav : '{{ $hasFav }}'}">
                                <span wire:click.prevent="fav('{{ $challenge->id }}')">
                                    <i class="fa fa-heart" style="font-size:18px;cursor: pointer;   " :class="{'text-red' : isFav == true}"></i>
                                </span>
                            </div>
                            <div>&bull;</div>
                            <div x-data="{isSpammed : '{{ $hasMarkedSpam }}'}">
                                <span wire:click.prevent="markAsSpam('{{ $challenge->id }}')" title="Report Abuse">
                        ...
                        @if ($challenge->created_at != $challenge->updated_at)
                            <div>&bull;</div>
                            <div class="text-gray-900">Edited: {{ $challenge->updated_at->diffForHumans() }}</div>
                        @endif
                    </div>
                    ...
                    <button
                        ...
                        <svg fill="currentColor" width="24" height="6"><path d="M2.97.061A2.969 2.969 0 000 3.031 2.968 2.968 0 002.97 6a2.97 2.97 0 100-5.94zm9.184 0a2.97 2.97 0 100 5.939 2.97 2.97 0 100-5.939zm8.877 0a2.97 2.97 0 10-.003 5.94A2.97 2.97 0 0021.03.06z" style="color: rgba(163, 163, 163, .5)"></svg>
                            ...
                            <li><a href="javascript:;" wire:click.prevent="deleteIdea('{{ $challenge->id }}')" class="hover:bg-gray-100 block transition duration-150 ease-in px-5 py-3">Delete Post</a></li>
                        </ul>
                    </button>
                    @endif
                    </div>
